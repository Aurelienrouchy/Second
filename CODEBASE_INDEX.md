...
| `recourse.ts` | Recours acheteur anti-fraude. `requestRefund` (buyer-only, rate-limit 5/min) : auto-refund idempotent `rf_buyer_<txId>` UNIQUEMENT si status ∈ `delivery_failed`/`lost` (signal transporteur), réutilise `issueTransactionRefund`, NE re-liste PAS l'article (colis perdu), `refund_in_progress`→`refunded`, notifie les deux. `reportTransactionProblem` (buyer-only, rate-limit 5/min) : « livré mais problème », AUCUN mouvement d'argent, pose `disputed=true`+`buyerReport`+status `disputed` (gel des fonds), crée doc `disputes`, log `ADMIN_REVIEW`, refuse un re-signalement |
...
| `trackingCheck.ts` | Filet de sécurité (toutes les 12h, paginé orderBy createdAt + startAfter, throttle ShipEngine) : poll les transactions `label_created` + `shipped`. 1er scan transporteur → `shipped` ; DELIVERED → contrat heldBalance (+7j) ; FAILURE → `delivery_failed` ; relance vendeur si `label_created` sans scan depuis 3j. Délègue à `utils/trackingTransition` |
