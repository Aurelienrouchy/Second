...
| `/complete-profile` | `complete-profile.tsx` | Étape consentement OBLIGATOIRE post-inscription (@pseudo + DOB + consents) |
...
| `walletService` | `walletService.ts` | Porte-monnaie (callables: activate, info, withdraw, pay) + `getWithdrawalRequests` (lecture owner-scopée des `withdrawal_requests`, F128) |
| `stripeAccountService` | `stripeAccountService.ts` | Compte vendeur Stripe Connect (callables: getStripeAccountStatus, uploadStripeIdentityDocument, addBankAccount) |
...
| `payments.ts` | Paiement Stripe Connect Custom (checkout, full in-app account onboarding with identity+bank, `addBankAccount` — F60a : remplace le compte via `default_for_currency:true` + purge des anciens externals + persiste `stripeBankAccountLast4`/`stripeBankAccountStatus` ; `uploadStripeIdentityDocument` (F59b, owner-only, rate-limit 5/min, secret STRIPE_SECRET_KEY) : base64→Buffer→`files.create(purpose:'identity_document')` front+back optionnel→`accounts.update(individual.verification.document)`, retourne les requirements rafraîchis ; `getStripeAccountStatus` (F62/F117) retourne le contrat complet `{status, chargesEnabled, payoutsEnabled, detailsSubmitted, requirementsCurrentlyDue, requirementsPastDue, disabledReason, currentDeadline, bankAccountLast4}` et persiste via `utils/stripeAccount`, tracking via `checkTrackingStatus` avec garde de statut + `applyTrackingOutcome`, meetup completion, status). `completeMeetupTransaction` (buyer OU seller, `meetup_confirmed`→`meetup_completed`, aucun argent). `reportMeetupNoShow` (buyer ou seller, meetup uniquement) : déverrouille l'article (`isSold:false`), fige la tx en `disputed`, ouvre un doc `disputes` (`type:'meetup_no_show'`) pour revue humaine — recours des deux parties. `adminRefundTransaction` (admin-only) : délègue au coeur partagé `utils/refund.ts` (`issueTransactionRefund`), idempotent `rf_admin_<txId>` refund Stripe plat (charge plateforme, single-rail, PAS de reverse_transfer) + débit vendeur pendingBalance/heldBalance/balance + status `refunded` (résout disputed/delivery_failed/lost/return_requested), re-liste l'article, **ferme les docs `disputes` liés** (`resolution:'refunded'`, F27/F88). `resolveDispute` (admin-only, rate-limit 20/min, F27/F88+F10+F26) : clôt un litige EN FAVEUR DU VENDEUR sans remboursement — ferme le(s) doc(s) `disputes` (`resolution:'dismissed'`), restaure `statusBeforeDispute`, `disputed:false`, libère le hold `disputeFreezeCents` heldBalance→balance (no-op meetup/return). `sellerCancelTransaction` (vendeur-only, rate-limit 10/min, F74) : annulation vendeur post-paiement AVANT 1er scan transporteur (statuts `paid`/`label_created` uniquement, refusé dès `shipped`) → `issueTransactionRefund` clé `rf_seller_<txId>` (remboursement acheteur + débit vendeur cascade) + re-liste l'article + notifie l'acheteur. `acceptMeetupOffer` : buyer/seller dérivés de l'ARTICLE (owner=vendeur, autre participant=acheteur, F9), accepté par le NON-émetteur de l'offre, idempotent si une tx meetup non-terminale existe déjà pour ce chat+buyer+article (F8) |
...
| `swaps.ts` | `expireStaleProposedSwaps` : expiration swaps stale (proposed + payment_pending non payés, 7j) → libère les items. `expireStalePostAcceptanceSwaps` (F51/F52, toutes les heures, index `status,updatedAt`) : déroule les swaps bloqués en `accepted`/`photos_pending`/`shipping` après 14j → refund top-up payé (idempotent `rf_swap_<id>`) + libère les items + statut `expired`. `alertAgingDisputes` (B9, toutes les 6h, index `swaps status,disputeOpenedAt` + `transactions status,disputedAt`) : surveille les litiges `disputed` (swaps + transactions) > 5j sans résolution admin → `writeAdminAlert` idempotent via stamp `disputeAlertedAt` (CF-only). N'auto-tranche jamais |
