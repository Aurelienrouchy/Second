...
exports.retryFailedOperations = void 0;
/**
...
function normalizeOp(doc) {
    const d = doc.data();
    const payload = d.payload && typeof d.payload === 'object' ? d.payload : {};
    // Legacy docs stored ids/reason at the top level; merge them into payload.
    const mergedPayload = Object.assign(Object.assign(Object.assign({}, payload), (d.paymentIntentId ? { paymentIntentId: d.paymentIntentId } : {})), (d.transactionId ? { transactionId: d.transactionId } : {}));
...
function backoffElapsed(op, now) {
    if (op.lastTriedAtMs == null)
        return true; // never tried → eligible now
 ...
 */
async function replayOp(op, stripe) {
    switch (op.type) {
        ...
        case 'admin_refund_failed': {
            ...
            const idempotencyKey = typeof op.payload.idempotencyKey === 'string' && op.payload.idempotencyKey
            ...
            try {
                await stripe.refunds.create(Object.assign({ payment_intent: paymentIntentId }, (isMixedCharge
                    ? {}
                    : { reverse_transfer: true, refund_application_fee: true })), { idempotencyKey });
        ...
        case 'payout_reversal_failed': {
            ...
            try {
                await stripe.payouts.cancel(payoutId, undefined, typeof stripeAccountId === 'string' && stripeAccountId
        ...
        case 'amount_mismatch': {
            if (op.payload.autoRefund === true && stripe) {
                ...
                if (typeof paymentIntentId === 'string' && paymentIntentId) {
                    try {
                        await stripe.refunds.create(Object.assign({ payment_intent: paymentIntentId }, (op.payload.isMixedCharge === true
                            ? {}
                            : { reverse_transfer: true, refund_application_fee: true })), { idempotencyKey: `rf_mismatch_${op.refId}` });
...
exports.retryFailedOperations = (0, scheduler_1.onSchedule)({
    ...
    for (let i = 0; i < eligible.length; i += LOT_SIZE) {
        ...
        const outcomes = await Promise.allSettled(lot.map(async (op) => {
            const outcome = await replayOp(op, stripe);
            const opRef = firebase_1.db.collection('failed_operations').doc(op.id);
            if (outcome === 'resolved') {
                await opRef.update({
                    status: 'resolved',
                    attempts: firebase_1.FieldValue.increment(1),
                    lastTriedAt: firebase_1.FieldValue.serverTimestamp(),
            ...
            if (nextAttempts >= MAX_ATTEMPTS) {
                await opRef.update({
                    ...
                    lastTriedAt: firebase_1.FieldValue.serverTimestamp(),
                    exhaustedAt: firebase_1.FieldValue.serverTimestamp(),
                });
...
//# sourceMappingURL=retryFailedOperations.js.map