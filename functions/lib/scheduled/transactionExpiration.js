...
exports.expireOrphanedTransactions = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        if (!paymentSnap.empty) {
            ...
            for (let i = 0; i < paymentSnap.docs.length; i += STRIPE_LOT_SIZE) {
                ...
                const results = await Promise.allSettled(lot.map((doc) => expirePendingPayment(doc, stripe)));
            ...
            logger.info(`[expireOrphanedTransactions] Expired ${pendingExpired}/${paymentSnap.size} pending_payment transactions (rest had in-flight PaymentIntents)`);
        }
    }
    catch (error) {
    ...
    try {
        ...
        if (!paidSnap.empty) {
            ...
            for (let i = 0; i < paidSnap.docs.length; i += STRIPE_LOT_SIZE) {
                ...
                const results = await Promise.allSettled(lot.map((doc) => refundPaidNotShipped(doc, stripe)));
                for (const r of results) {
                    if (r.status === 'fulfilled' && r.value) {
                        paidExpired++;
                        totalExpired++;
            ...
            logger.info(`[expireOrphanedTransactions] Expired ${paidExpired}/${paidSnap.size} paid-not-shipped transactions (7d)`);
        }
    }
    catch (error) {
    ...
    // =========================================================================
    try {
        const stuckSnap = await firebase_1.db
            .collection('transactions')
        ...
        if (!stuckSnap.empty) {
            ...
            logger.info(`[expireOrphanedTransactions] Resumed ${resumed}/${stuckSnap.size} interrupted refunds (refund_in_progress)`);
        }
    }
    catch (error) {
 ...
 */
async function expirePendingPayment(doc, stripe) {
    ...
    try {
        ...
        const expired = await firebase_1.db.runTransaction(async (tx) => {
            ...
            const articleRef = txData.articleId
                ? firebase_1.db.collection('articles').doc(txData.articleId)
                : null;
            if (articleRef) {
 ...
 */
async function refundPaidNotShipped(doc, stripe) {
    const transactionId = doc.id;
    const data = doc.data();
    ...
    try {
        ...
        // -----------------------------------------------------------------------
        const intent = await firebase_1.db.runTransaction(async (tx) => {
            const txSnap = await tx.get(doc.ref);
            const txData = txSnap.data();
            if (!txData) {
                return { proceed: false, existingRefundId: null };
            }
            ...
            if (txData.status === 'paid') {
                tx.update(doc.ref, {
                    ...
                    refundStartedAt: firebase_1.FieldValue.serverTimestamp(),
                });
                return {
                    proceed: true,
                    existingRefundId: typeof txData.stripeRefundId === 'string' ? txData.stripeRefundId : null,
                };
            }
        ...
        // -----------------------------------------------------------------------
        let stripeRefundId = intent.existingRefundId;
        if (data.stripePaymentIntentId && stripe && !stripeRefundId) {
            try {
                ...
                const refund = await stripe.refunds.create(Object.assign({ payment_intent: data.stripePaymentIntentId }, (isMixedCharge
                    ? {}
        ...
        const confirmed = await firebase_1.db.runTransaction(async (tx) => {
            const txSnap = await tx.get(doc.ref);
            const txData = txSnap.data();
            ...
            tx.update(doc.ref, Object.assign({ status: 'refunded', cancelledAt: firebase_1.FieldValue.serverTimestamp(), cancelReason: 'seller_did_not_ship_7d', refundedAt: firebase_1.FieldValue.serverTimestamp() }, (stripeRefundId ? { stripeRefundId } : {})));
