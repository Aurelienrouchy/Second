 ...
 */
async function refundPaidNotShipped(doc, _stripe) {
    const transactionId = doc.id;
    const data = doc.data();
    ...
    try {
        ...
        const intent = await firebase_1.db.runTransaction(async (tx) => {
            ...
            const txData = txSnap.data();
            if (!txData)
                return { proceed: false, preData: null };
            if (txData.status === 'paid') {
                ...
                return { proceed: true, preData: txData };
            }
        ...
        // -----------------------------------------------------------------------
        await (0, refund_1.issueTransactionRefund)(transactionId, intent.preData, {
        ...
        await doc.ref
            ...
            .catch((err) => logger.warn('[expireOrphanedTransactions] failed to stamp cancel fields after refund', {
            transactionId,
            error: err instanceof Error ? err.message : err,
        }));
