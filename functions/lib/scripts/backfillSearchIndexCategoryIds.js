...
async function run() {
    ...
    for (const idxDoc of searchIndexSnap.docs) {
        ...
        const articleId = idxData.articleId || idxDoc.id;
        const articleSnap = await db.collection('articles').doc(articleId).get();
        if (!articleSnap.exists) {
            missingArticle++;
            continue;
        }
        const articleCategoryIds = ((_a = articleSnap.data()) === null || _a === void 0 ? void 0 : _a.categoryIds) || [];
        const existing = idxData.categoryIds || [];
        // Skip si déjà identique (idempotence)
        const same = existing.length === articleCategoryIds.length &&
    ...
    console.log(`[backfillSearchIndexCategoryIds] done — scanned=${scanned} updated=${updated} skipped=${skipped} missingArticle=${missingArticle}`);
}
run()
    .then(() => process.exit(0))
    .catch((err) => {
...
//# sourceMappingURL=backfillSearchIndexCategoryIds.js.map