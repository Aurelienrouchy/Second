...
exports.TOP_LEVEL_CATEGORIES = exports.FLAT_CATEGORIES = exports.CATEGORIES = void 0;
exports.generateIdFromPath = generateIdFromPath;
exports.findCategoryById = findCategoryById;
exports.getCategoryPath = getCategoryPath;
exports.getCategoryLabelFromIds = getCategoryLabelFromIds;
exports.getLeafCategoryLabel = getLeafCategoryLabel;
exports.flattenCategories = flattenCategories;
exports.findFlatCategoryById = findFlatCategoryById;
exports.getLeafCategories = getLeafCategories;
exports.generateCategoryPromptSection = generateCategoryPromptSection;
exports.generateStep1Prompt = generateStep1Prompt;
exports.getSubcategoriesForTopLevel = getSubcategoriesForTopLevel;
exports.getLeafCategoriesForTopLevel = getLeafCategoriesForTopLevel;
exports.generateStep2CategoryPrompt = generateStep2CategoryPrompt;
exports.findCategoryByLabel = findCategoryByLabel;
exports.estimateTokenCount = estimateTokenCount;
exports.generateCompactCategoryPrompt = generateCompactCategoryPrompt;
exports.findCategoryByLabelFuzzy = findCategoryByLabelFuzzy;
exports.getCategoryStats = getCategoryStats;
// Helper to generate ID from path (exported for potential use in scripts)
function generateIdFromPath(parts) {
    return parts
        .map(p => p.toLowerCase()
 ...
 */
function findCategoryById(id, nodes = exports.CATEGORIES) {
 ...
 */
function getCategoryPath(id, nodes = exports.CATEGORIES, path = []) {
    for (const node of nodes) {
 ...
 */
function getCategoryLabelFromIds(ids, separator = ' > ') {
    if (!ids || ids.length === 0)
        return '';
    const labels = [];
    for (const id of ids) {
 ...
 */
function getLeafCategoryLabel(ids) {
    if (!ids || ids.length === 0)
        return '';
...
function flattenCategories(nodes = exports.CATEGORIES, path = [], labels = []) {
    const result = [];
    for (const node of nodes) {
        const currentPath = [...path, node.id];
        ...
        let gender = 'unisex';
        if (currentPath[0] === 'women')
            gender = 'women';
        else if (currentPath[0] === 'men')
            gender = 'men';
        else if (currentPath[0] === 'kids')
            gender = 'kids';
        result.push({
            id: node.id,
            label: node.label,
...
exports.FLAT_CATEGORIES = flattenCategories();
/**
 * Find flat category by ID
 */
function findFlatCategoryById(id) {
    return exports.FLAT_CATEGORIES.find(cat => cat.id === id);
}
/**
...
function getLeafCategories() {
    ...
    for (const cat of exports.FLAT_CATEGORIES) {
    ...
    return exports.FLAT_CATEGORIES.filter(cat => !parentIds.has(cat.id));
}
/**
 * Generate category prompt section for AI (Gemini) - FULL LIST
 * Warning: This generates a lot of tokens!
 */
function generateCategoryPromptSection() {
...
function generateStep1Prompt() {
    const categories = exports.TOP_LEVEL_CATEGORIES.map(c => `"${c.id}" = ${c.label} (${c.description})`).join('\n');
 ...
 */
function getSubcategoriesForTopLevel(topLevelId) {
    return exports.FLAT_CATEGORIES.filter(cat => cat.path[0] === topLevelId && cat.path.length > 1);
}
/**
 * Get leaf categories for a specific top-level category
 */
function getLeafCategoriesForTopLevel(topLevelId) {
 ...
 */
function generateStep2CategoryPrompt(topLevelId) {
 ...
 */
function findCategoryByLabel(label, topLevelId) {
    ...
    if (match)
        return match;
    // Partial match
    match = candidates.find(cat => cat.label.toLowerCase().includes(normalized) ||
 ...
 */
function generateCompactCategoryPrompt() {
    const sections = [];
    for (const topCat of exports.TOP_LEVEL_CATEGORIES) {
 ...
 */
function findCategoryByLabelFuzzy(label, topLevelHint) {
    if (!label)
        return undefined;
    const normalized = label.toLowerCase().trim()
        .normalize('NFD').replace(/[\u0300-\u036f]/g, ''); // Remove accents
    ...
    let match = candidates.find(cat => cat.label.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') === normalized);
    if (match)
        return match;
    // 2. Label starts with input
    match = candidates.find(cat => cat.label.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').startsWith(normalized));
    if (match)
        return match;
